...
# 31. **На что влияет регистр первой буквы поля или метода в структуре?**
    - Точно так же, как и для переменных и функций, если поле или метод структуры начинается с заглавной буквы, оно/он будет доступно/доступен вне пакета (public). Если с маленькой, то только внут
...
# 41. **Что такое `panic`? Что происходит в программе, когда вылетает паника?**
   - `Panic` — это встроенная функция в Go, которая вызывает аварийное завершение программы. При вызове `panic` начинается "размотка стека", при которой вызыва

ются все функции `defer`. Если паника не обработана, программа завершается с ошибкой.
